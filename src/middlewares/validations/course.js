...
exports.checkstudentsInCourse = async (req, res, next) => {
    ...
    if (courseExist.length > 0) {
        ...
        loggerUtil.error(
            `Students with ID (${message}) ${courseExist.length > 1 ? 'have' : 'has'} Already Registered this Course`
        );
        next(
            new HttpException(
                422,
                `Students with ID (${message}) ${
                    courseExist.length > 1 ? 'have' : 'has'
                } Already Registered this Course`
            )
        );
    } else {
        next();
